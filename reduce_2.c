/*
   ...
   omp_nbody_red_forces_cyclic.c program

   omp_nbody_red__force_cyclic solver program. The only modifications to this program are the omp for loops used to compute force uses cyclic partitioning 
   and the definition of macro NO_OUTPUT. This marco makes it so the program no longer generates particle computation output to the command line, it only
   outputs the execution time in seconds. This is done to keep in line with the Pacheco text book which states "[n-body solvers] run on one of our systems with no I/O"
   when collecting their results, hence to stay as accurate and consistent to the evaluation in the book, the same is achieved through NO_OUTPUT macro.
   This runtime output by this program is what is used in the analysis paper.

   I have supplied a comiple_script.sh which will compile this omp program (as well as the other 3 nbody solvers) and also a run.sh script
   which runs all nbody solver programs (this one included).

   The run.sh script will redirect the runtime output to the corresponding file in the perfdata folder. Each nbody solver program has its own data
   collection text file (eg. runtimes from this red_forces_cyclic solver will be redirected into reduced_forces_cyclic.txt).

   This reduced_forces_cyclic nbody solver uses default partitioning (block partitioning) for all the omp for loops EXCEPT the omp for loop for computing forces.
   line 170 shows the omp for pragma with (static, 1) clause, which will cause that omp for directive to run the for-loop responsible for compute_force() with cyclic partitioning.

   Data is collected by running each n-body solver with constant (mirroring the experiment conditions from the Pacheco textbook): 
   <number of particles> 400
   <number of timesteps>  1000
   <size of timestep>  0.01
   <output frequency> 100
   <g> (randomly generated initial conditions)

   The only variables changed are number of threads and also partitioning technique. This is so we can accurately measure the effect of partitioning technique on runtime.
*/
...
#define NO_OUTPUT //define NO_OUTPUT macro so no particle data is output, on runtime
...
#        endif
          ...
          * Compute_force(n-2, . . .) */
#        pragma omp for schedule(static,1) //this is cyclic partitioning by using the (static, 1) clause to force round-robin (cyclic) partitioning on this for-loop responsible for compute_force()
