/*
   ...
   omp_nbody_red_all_cyclic.c program

   omp_nbody_red_all_cyclic solver program. The only modifications to this program are all omp for loops use cyclic partitioning 
   ...
   collection text file (eg. runtimes from this red_all_cyclic solver will be redirected into reduced_all_cyclic.txt).

   This reduced_all_cyclic nbody solver uses cyclic parititiong for all the omp for loops. Hence all omp for pragmas in the main() have an adiitional (static, 1) clause
   to force the iteration partitioning to be cyclic instead of block.
...
#        pragma omp for schedule(static,1) //this is cyclic partitioning by using the (static, 1) clause to force round-robin (cyclic) partitioning
