/*
   ...
   omp_nbody_red_default solver program. The only modifications to this program are all omp for loops use default block partitioning and the definition of macro NO_OUTPUT.
   This marco makes it so the program no longer generates particle computation output to the command line, it only
   ...
   collection text file (eg. runtimes from this red_basic solver will be redirected into reduced_default_shed.txt).

   This reduced_default nbody solver uses default partitioning (block partitioning) for all the omp for loops.
...
#        pragma omp for //all omp for loops use default scheduling
